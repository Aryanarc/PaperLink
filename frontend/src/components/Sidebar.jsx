...
export default function Sidebar() {
  ...
  const handleArxivSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('arXiv import failed');
      }
      ...
      setError('Failed to import from arXiv. Please check the URL.');
      ...
      <div className="p-6">
        ...
        >
          <div className="flex flex-col items-center gap-3">
            ...
            <label 
              htmlFor="file-upload"
              className={`px-4 py-2 bg-panel hover:bg-border/50 rounded-lg text-sm text-text cursor-pointer transition-colors ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              Choose File
            </label>
          </div>
...
function DocumentItem({ title, pages, status, authors }) {
  const statusConfig = {
  ...
  return (
    <div className="group p-4 bg-panel/50 hover:bg-panel border border-border/50 hover:border-border rounded-xl transition-all duration-200 cursor-pointer hover:scale-[1.01]">
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        <div className={`px-2 py-1 ${config.bg} rounded-lg flex items-center gap-1`}>
      ...
      <div className="flex items-center gap-4 text-xs text-muted">
        ...
        <button className="opacity-0 group-hover:opacity-100 text-primary hover:underline transition-opacity">
          View
        </button>
      </div>
    </div>
